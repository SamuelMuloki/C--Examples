...
public:
    ...
    void displayMessage() const {
        // call getCourseName to get the name of Gradebook's course
        std::cout << "Welcome to the GradeBook for\n" << getCourseName() << "!" << std::endl;
    }

    // perform operations on the data (non modify the data)
    void processGrades() const {
        outputGrades(); // output the grades

        // call the function getAverage to calculate the average grade
        std::cout << std::setprecision(2) << std::fixed;
        std::cout << "\nClass avarage is " << getAverage() << std::endl;
    ...
    void outputGrades() const {
        ...
        for (size_t student{0}; students < grades.size(); ++student) {
